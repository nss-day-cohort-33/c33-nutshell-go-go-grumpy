...
let currentUserId = sessionStorage.getItem("userId")
  getNewsData(currentUserId).then(articles => listArticles(articles));
...
function newsListener() {
  document.querySelector("#saveNews").addEventListener("click", () => {
    ...
    let newNews = newsFactory(createTitle, createSummary, createURL, currentUserId);
...
function newsFactory(name, summary, url, currentUserId) {
  return {
    name: name,
    summary: summary,
    url: url,
...
function createEditForm(article) {
  ...
  newsContainer.innerHTML = `
    <fieldset>
    <label for="newsTitle">Article Title</label>
    <input type="text" class="title" name="newsTitle" id="articleTitle" value=${
      article.name
    }>
    </fieldset>
    <fieldset>
    <label for="newsSummary">Summary</label>
    <input type="text" class="summary" name="newsSummary" id="articleSummary" value=${
      article.summary
    }>
    </fieldset>
    <fieldset>
  ...
  function createSaveChangesBtn() {
    ...
    saveEditBtn.addEventListener("click", () => {
      ...
      console.log(editURL);
      let editedNews = newsFactory(editTitle, editSummary, editURL, currentUserId);
...
export { createArticleForm, newsListener, displayNewsArticles, listArticles };
